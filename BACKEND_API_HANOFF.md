...
If you'd like I can:
- Produce an automated API contract (OpenAPI/Swagger-like) inferred from the frontend's expectations.
- Run a pass to collect example JSON shapes from current backend (if you provide a staging base URL).

---
Generated by frontend scan on branch `feat/restore-profile-settings`.
